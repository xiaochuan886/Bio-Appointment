...
import { initializeConnections, query } from '../src/db/connection';
import { AuthService } from '../src/services/auth';
import { ApiService } from '../src/services/api';
import type { CreateAppointmentInput, LoginCredentials, RegisterCredentials } from '../src/types/types';
...
app.get('/api/health', async (req, res) => {
  try {
    const healthStatus = await (await import('../src/db/connection')).healthCheck();
    res.json({
      status: 'healthy',
...
app.get('/api/services', async (req, res) => {
  try {
    ...
    const services = await ApiService.getServices(category as string);
    res.json(services);
  } catch (error) {
    res.status(500).json({
...
app.get('/api/resources', async (req, res) => {
  try {
    ...
    const resources = await ApiService.getResources(type as string, status as string);
    res.json(resources);
  } catch (error) {
    res.status(500).json({
...
app.get('/api/schedules', async (req, res) => {
  try {
    await ensureDatabase();
    const filters = req.query;
    const schedules = await ApiService.getSchedules(filters);
    res.json(schedules);
  } catch (error) {
    res.status(500).json({
...
app.get('/api/task-executions', async (req, res) => {
  try {
    await ensureDatabase();
    const filters = req.query;
    const tasks = await ApiService.getTaskExecutions(filters);
    res.json(tasks);
  } catch (error) {
    res.status(500).json({
...
app.get('/api/resources/availability', async (req, res) => {
  try {
    ...
    const availability = await ApiService.getResourceAvailability(
      date as string,
  ...
  try {
    ...
    try {
      ...
      const deptIdsToSync = selected_departments.length > 0
        ? selected_departments
        : deptData.result.map((d: any) => d.dept_id.toString());
            ...
            try {
              ...
              if (existingUser.rows.length > 0) {
                ...
                if (strategy === 'dingtalk_first') {
                  ...
                  const deptName = deptData.result?.find((d: any) => d.dept_id === user.dept_id_list[0])?.name;
                  await query(
                    `UPDATE profiles 
