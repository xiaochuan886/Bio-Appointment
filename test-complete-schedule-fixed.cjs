...
async function testCompleteScheduleFix() {
  ...
  try {
    ...
    const startHour = 10;
    const startMinute = 0;
    const endMinutes = startHour * 60 + startMinute + newDuration;
    ...
    const testScheduleData = {
      ...
      scheduled_time_start: `${String(startHour).padStart(2, '0')}:${String(startMinute).padStart(2, '0')}:00`,
    ...
    console.log(`📊 时间范围: ${testScheduleData.scheduled_time_start} - ${testScheduleData.scheduled_time_end}`);
