...
async function testDoctorScheduleCreation() {
  ...
  try {
    ...
    // 2. 查找最近确认的预约
    console.log('\n2. 查找最近确认的预约...');
    const appointmentsResponse = await makeRequest('/api/appointments?status=confirmed&limit=50');
    
    if (appointmentsResponse.statusCode !== 200) {
      console.error('❌ 查询预约失败:', appointmentsResponse.data);
      return;
    }
    ...
    if (Array.isArray(appointmentsResponse.data)) {
      ...
      console.error('❌ 无法解析预约数据:', appointmentsResponse.data);
      return;
    }

    // 过滤出赵医生的预约
    const doctorAppointments = appointments.filter(apt => apt.doctor_id === doctor.id);
    console.log(`✅ 找到 ${doctorAppointments.length} 个赵医生的已确认预约:`);
    doctorAppointments.forEach((apt, index) => {
      console.log(`  ${index + 1}. ${apt.customer_name} - ${apt.requested_date} ${apt.requested_time_start} (ID: ${apt.id})`);
    });

    // 如果没有找到已确认的预约，创建一个测试预约
    if (doctorAppointments.length === 0) {
      console.log('⚠️ 没有找到赵医生的已确认预约，创建测试预约...');
    ...
    for (const apt of doctorAppointments) {
      ...
      const schedulesResponse = await makeRequest(`/api/schedules?appointment_id=${apt.id}`);
      
      if (schedulesResponse.statusCode !== 200 || !schedulesResponse.data.success) {
        console.error(`❌ 查询排班失败:`, schedulesResponse.data);
        continue;
      }
    ...
    if (doctorSchedulesResponse.statusCode !== 200 || !doctorSchedulesResponse.data.success) {
      console.error('❌ 查询医生排班失败:', doctorSchedulesResponse.data);
      return;
    }

    const doctorSchedules = doctorSchedulesResponse.data.data;
    console.log(`✅ 赵医生共有 ${doctorSchedules.length} 个排班:`);
    doctorSchedules.forEach((schedule, index) => {
      console.log(`  ${index + 1}. 日期: ${schedule.date}, 时间: ${schedule.start_time}-${schedule.end_time}, 状态: ${schedule.status}, 预约ID: ${schedule.appointment_id}`);
    });
    ...
    if (confirmResponse.statusCode !== 200 || !confirmResponse.data.success) {
      console.error('❌ 确认预约失败:', confirmResponse.data);
      return;
    }
    ...
    if (newSchedulesResponse.statusCode !== 200 || !newSchedulesResponse.data.success) {
      console.error('❌ 查询新排班失败:', newSchedulesResponse.data);
      return;
    }

    const newSchedules = newSchedulesResponse.data.data;
    if (newSchedules.length === 0) {
      console.log(`  ❌ 确认预约后没有创建排班记录`);
    } else {
      console.log(`  ✅ 确认预约后创建了 ${newSchedules.length} 个排班记录:`);
