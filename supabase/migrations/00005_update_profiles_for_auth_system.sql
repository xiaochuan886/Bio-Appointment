...
## 4. 更新 RLS 策略
- 超级管理员拥有所有权限
- 用户可以查看和更新自己的信息（除了 role 和 status）

## 5. 创建自动同步触发器
- 当用户注册后自动同步到 profiles 表
- 第一个用户自动成为超级管理员
*/

-- 1. 添加 super_admin 到 user_role 枚举
...
-- 3. 修改 profiles 表结构
-- 添加 username 字段（临时允许 NULL，稍后更新）
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS username text;

-- 添加 department 字段
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS department text;

-- 重命名 name 为 full_name（如果还没有 full_name）
DO $$ 
BEGIN
  IF NOT EXISTS (
...
-- 为 username 生成默认值（基于 email 或 id）
UPDATE profiles 
