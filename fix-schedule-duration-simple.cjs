...
// 查找并替换排班创建代码
const insertSchedulePattern = /const result = await pool\.query\(\s*`INSERT INTO schedules \(appointment_id, scheduled_date, scheduled_time_start, scheduled_time_end, room_id, nurse_id, notes, status\)\s*VALUES \(\$1, \$2, \$3, \$4, \$5, \$6, \$7, 'scheduled'\)\s*RETURNING \*`,\s*\[appointment_id, scheduled_date, scheduled_time_start, scheduled_time_end, room_id, nurse_id, notes\]\s*\);/;

if (insertSchedulePattern.test(content)) {
  console.log('✅ [修复] 找到排班创建代码');
  
  // 新的排班创建代码，添加时长计算和预约更新
  ...
  // 替换原有代码
  content = content.replace(insertSchedulePattern, newInsertCode);
...
// 查找并替换医生排班创建代码
const insertDoctorSchedulePattern = /const scheduleResult = await pool\.query\(\s*`INSERT INTO schedules \(appointment_id, scheduled_date, scheduled_time_start, scheduled_time_end, doctor_id, status, created_at, updated_at\)\s*VALUES \(\$1, CURRENT_DATE, \$3, \$4, \$5, 'scheduled', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP\)\s*RETURNING \*`,\s*\[updatedAppointment\.id, updatedAppointment\.requested_time_start, updatedAppointment\.requested_time_end, finalDoctorId\]\s*\);/;

if (insertDoctorSchedulePattern.test(content)) {
  console.log('✅ [修复] 找到医生排班创建代码');
  
  // 新的医生排班创建代码，添加时长计算和预约更新
  ...
  // 替换原有代码
  content = content.replace(insertDoctorSchedulePattern, newDoctorInsertCode);
...
// 查找并替换排班更新代码
const updateSchedulePattern = /const result = await pool\.query\(\s*`UPDATE schedules SET \${updateFields\.join\(', '\)}, updated_at = CURRENT_TIMESTAMP\s*WHERE id = \$\{paramIndex\}\s*RETURNING \*`,\s*values\s*\)\s*;/;

if (updateSchedulePattern.test(content)) {
  console.log('✅ [修复] 找到排班更新代码');
  
  // 新的排班更新代码，添加时长计算和预约更新
  ...
  // 替换原有代码
  content = content.replace(updateSchedulePattern, newUpdateCode);
