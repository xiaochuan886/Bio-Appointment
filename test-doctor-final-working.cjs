...
async function testDoctorConfirm() {
  ...
  try {
    ...
    const createResponse = await makeRequest('/api/appointments', 'POST', testAppointment, authToken);
    ...
    // 3. 确认预约
    console.log('\n3. 确认测试预约...');
    const confirmResponse = await makeRequest(`/api/appointments/${newAppointment.id}/doctor-confirm`, 'PUT', null, authToken);
    ...
    // 4. 检查是否创建了排班
    console.log('\n4. 检查是否创建了排班...');
    const newSchedulesResponse = await makeRequest(`/api/schedules?appointment_id=${newAppointment.id}`, 'GET', null, authToken);
    ...
    // 5. 检查医生的所有排班
    console.log('\n5. 检查赵医生的所有排班...');
    const doctorSchedulesResponse = await makeRequest(`/api/schedules/doctor?doctor_id=${doctor.id}`, 'GET', null, authToken);
